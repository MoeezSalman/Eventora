...
export default function EventDetailPage() {
  ...
  if (loading) {
    return <div style={{ color: "#fff", padding: 20 }}>Loading event...</div>;
  }
  ...
  if (!event) {
    return <div style={{ color: "#fff", padding: 20 }}>Event not found</div>;
  }
...
const startingPrice =
  Number(event.price) ||
  Math.min(
    ...(event.ticketTiers?.map((tier) => Number(tier.price) || 0).filter((p) => p > 0) || [0])
  );
  ...
  const highlights = [
  {
    icon: "📅",
    label: "EVENT DATE",
    value: eventDate ? new Date(event.eventDate).toLocaleDateString() : "TBA",
  },
  {
  icon: "🕒",
  label: "GATE OPENS",
  ...
  },
  {
    icon: "💰",
    label: "PRICE",
    value: startingPrice > 0 ? `Rs. ${startingPrice.toLocaleString()}` : "Free",
  },
];
      ...
      <div style={{ minHeight: "100vh", background: "#0d0d14" }}>
        ...
        <div className="page-grid">
          <div>
            ...
            >
              <div
                style={{
                  ...
                  inset: 0,
                  backgroundImage:
                    "radial-gradient(ellipse at 80% 30%,rgba(168,85,247,0.3) 0%,transparent 60%)",
                  pointerEvents: "none",
                }}
              />
              <div
              ...
              <div style={{ position: "relative", zIndex: 1 }}>
                ...
                <div className="hero-meta">
                  <span>🕒 Gate Opens: {gateOpens || "TBA"}</span>
              ...
              {["About", "Tickets", "Organizer"].map((tab) => (
                <button
                  ...
                  style={{
                    ...
                    cursor: "pointer",
                    borderBottom:
                      activeTab === tab ? "2px solid #a855f7" : "2px solid transparent",
                    marginBottom: -1,
                    whiteSpace: "nowrap",
                  }}
                >
            ...
            {activeTab === "Tickets" && (
              <p style={{ color: "#888", fontSize: 14 }}>Ticket tiers will appear here.</p>
            )}

            {activeTab === "Organizer" && (
              <p style={{ color: "#888", fontSize: 14 }}>Organizer info will appear here.</p>
            )}
          </div>
          ...
          <div className="booking-card">
            ...
            >
              <span style={{ fontSize: 12, color: "#a855f7", fontWeight: 600 }}>
                📅 {eventDate ? new Date(eventDate).toDateString() : "Date TBA"}
              </span>
              
            </div>

            <p style={{ fontWeight: 700, fontSize: 15, marginBottom: 2 }}>{eventTitle}</p>
            <p style={{ fontSize: 12, color: "#888", marginBottom: 18 }}>📍 {eventVenue}</p>
...
<p style={{ fontSize: 28, fontWeight: 800, letterSpacing: "-1px" }}>
  {startingPrice > 0 ? `Rs. ${startingPrice.toLocaleString()}` : "Free"}
</p>
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 18 }}>
              ...
              <span style={{ fontWeight: 700, fontSize: 15 }}>
                {startingPrice > 0 ? `Rs. ${(startingPrice * tickets).toLocaleString()}` : "Free"}
              </span>
            </div>

            <button
              onClick={() => navigate(`/seat-selection/${event._id || id}`)}
              style={{
                width: "100%",
                padding: 13,
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 18 }}>
              {[
                ...
                "📱 QR code pass on booking",
              ].map((t) => (
                <span key={t} style={{ fontSize: 12, color: "#666" }}>
                  {t}
                </span>
              ))}
            </div>
